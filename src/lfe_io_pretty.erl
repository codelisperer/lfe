...
term([map|MapBody], D, I, L) ->                 %Special case map form
    Fun = fun ([K,V]) -> {K,V} end,             %Convert to tuple pairs
    Mcs = map_body(lists:map(Fun, MapBody), D, I+5, L),
    ["(map ",Mcs,$)];
...
map_assoc({K,V}, CurL, D, I, L) ->
    ...
    if CurL+Kl+Vl < L-10 ->                     %Both fit on current line
            {curr_line,[$(,Kcs,$\s,Vcs,$)],Kl+3+Vl};
       I+Kl+Vl < L-10 ->                        %Both fit on one line
            {one_line,[$(,Kcs,$\s,Vcs,$)],Kl+3+Vl};
       true ->                                  %On separate lines
            ...
            {sep_lines,[$(,Ks],[Vs,$)]}         %Add the parentheses here
    end.
